...
from .codegen_device_driver import cuda_kernel_driver
from .codegen_device_driver import cuda_kernel_header
    ...
    def generate(self, is_inference):
        ...
        if not V.graph.aot_mode:
            for kernel in chain(
                ...
                self.prefix.writeline(maybe_hipify_code_wrapper(f"static CUfunction {kernel} = nullptr;"))
            self.prefix.writeline("\n")
    ...
    def generate_args_decl(self, call_args):
        ...
        for arg in call_args:
            ...
            if isinstance(arg, (sympy.Integer, sympy.Symbol, SymbolicCallArg)):
                ...
                if config.abi_compatible:
                    self.writeline(maybe_hipify_code_wrapper(f"CUdeviceptr {var_name};"))
                    self.writeline(
                        f"AOTI_TORCH_ERROR_CODE_CHECK(aoti_torch_get_data_ptr({arg}, reinterpret_cast<void**>(&{var_name})));"
                    )
                else:
                    self.writeline(
                        maybe_hipify_code_wrapper(f"CUdeviceptr {var_name} = reinterpret_cast<CUdeviceptr>({arg}.data_ptr());")
                    )
